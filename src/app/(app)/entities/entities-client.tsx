...
export function EntitiesClient() {
  const [selectedId, setSelectedId] = useState<string>("ai-crm");
  const [typeFilter, setTypeFilter] = useState<EntityNode["type"] | "all">("all");

  const satellites = useMemo(
    () => ENTITY_NODES.filter((n) => n.id !== "novacrm"),
    []
  );
  ...
  const positions = useMemo(() => {
    ...
    map.set("novacrm", { x: CENTER, y: CENTER });
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Entity Graph"
        description="What answer engines associate with NovaCRM — the concepts, audiences, competitors, and sources that co-occur in AI answers. Stronger associations sit closer to the center."
      />
          ...
          <CardContent>
            <div className="mx-auto max-w-[560px]">
              <svg
                ...
                className="w-full"
                role="img"
                aria-label="Entity association graph centered on NovaCRM"
              >
                {/* Orbit rings */}
                {[100, 160, 220].map((r) => (
                  <circle
